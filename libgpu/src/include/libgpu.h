...
bool libgpu_gpu_clear(struct libgpu_gpu* gpu, uint64_t id, float rgba[4], uint32_t minx, uint32_t maxx, uint32_t miny, uint32_t maxy);
...
bool libgpu_execute_graphics_pipeline(struct libgpu_gpu* gpu, uint64_t vs, uint64_t fs, uint64_t vb, uint64_t format, uint64_t ubo, uint64_t output_tex, size_t num_inputs);
