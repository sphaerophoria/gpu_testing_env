...
const camera_fov = std.math.pi / 8.0;

pub const model_obj = @embedFile("model_renderer/model.obj");
pub const model_img = @embedFile("model_renderer/model.png");

pub fn getTransform(aspect: f32, y_angle: f32) Mat4 {
    const perspective = Mat4.perspective(camera_fov, 0.1, aspect);
    return perspective.matmul(Mat4.translation(0.0, 0.0, -10.0).matmul(Mat4.rotAroundY(y_angle).matmul(Mat4.rotAroundX(0.75 + std.math.pi))));
}
...
pub const Model = struct {
    ...
    pub fn deinit(self: *Model, alloc: Allocator) void {
        alloc.free(self.verts);
        alloc.free(self.uvs);
        alloc.free(self.faces);
    }

    pub fn load(alloc: Allocator, data: []const u8) !Model {
    ...
    pub fn makeUploadVertexBuffer(model: Model, alloc: Allocator) ![]f32 {
        ...
        return try vertex_buffer.toOwnedSlice();
    }
};
